...
- 用户可以打开分销中心。
- 管理员可以打开分销管理。
- 分销中心邀请链接应显示为 `https://tokenrai.com/register?aff=<code>`。如果仍显示 `127.0.0.1` 或内网地址，请在后台系统设置或支付设置中确认 `服务器地址` 已填写为 `https://tokenrai.com`，并检查反向代理是否正确传递 `X-Forwarded-Proto` 与 `X-Forwarded-Host`。
- 分销中心里的代理管理、全部分销者汇总、全部奖励记录、全部邀请用户、奖励记录、邀请用户表格均可横向和纵向滚动查看完整数据。
